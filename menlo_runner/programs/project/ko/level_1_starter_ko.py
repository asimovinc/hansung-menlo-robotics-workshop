...
class ScannedDetection:
    """카메라 프레임을 찍을 때의 head pose를 함께 저장한 색상 detection입니다."""
    ...
    def full_bearing_deg(self) -> float:
        """이미지 각도에 head yaw를 더한 body-relative bearing입니다."""
...
# ---------------------------------------------------------------------------
# Level 1은 go_to를 사용할 수 있지만, observation에서 직접 추정한 좌표만
# 사용할 수 있습니다. entity ID, scene_state, 정답 object coordinate는 사용하지 마세요.
...
def estimate_target_xy_from_observation(observation: Observation, target_color: str | None) -> tuple[float, float] | None:
    ...
    - 가능하면 detection.full_bearing_deg를 사용해 head yaw를 포함하세요.
...
async def go_to_xy(ctx: Any, x: float, y: float) -> Any:
    """coordinate-based go_to입니다. 직접 추정한 x/y에만 사용하세요."""
    return await ctx.invoke(
        "go_to",
        {
            "target": {
...
async def execute_decision(
    ...
    - pick/place 전에는 robot이 의도한 target 근처에 있는지 검증하세요.
    - recover/skip/stop에 대한 팀의 정책을 구현하세요.
    """
...
async def run(ctx: Any) -> None:
    print(TASK)
    print("Running Level 1 coordinate-guided project starter")
